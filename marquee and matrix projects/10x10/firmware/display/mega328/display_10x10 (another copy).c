...
ISR(TIMER0_COMPA_vect) 
{
    ...
    if (r==c)
    {
    	setPort( getPinstate(1<<N) );
    	setDDR( getPinstate(((uint32_t)1<<(c))| (1<<N)) );
    }
    else
    {
        setPort(getPinstate( (~((uint32_t)1<<r))));
        setDDR(getPinstate( (((uint32_t)1<<c)|((uint32_t)1<<r))));
    }
}
