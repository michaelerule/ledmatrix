...
ISR(TIM0_OVF_vect) {
    ...
    uint8_t d = p>>4;
    uint32_t phaseshift = 64*period_estimate/512;
    p&=0xf;
    DDRB =0;
    uint16_t data = (disp[scanI]>>l)&1;
    if (data)// && timer > phaseshift && timer < (phaseshift + period_estimate)/2)
    {
        PORTB=p|0x10;
        DDRB =d;
    }
