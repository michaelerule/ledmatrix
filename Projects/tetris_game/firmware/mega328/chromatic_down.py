...
print '#define  CHROM_DOWN_NNOTES  %d'%len(notes)
print '#define  CHROM_DOWN_NUNIQUE %d'%len(unique)
print '#define  CHROM_DOWN_NPITCH  %d'%len(pitches)
print 'uint16_t CHROM_DOWN_pitches[NPITCH]  = {'+', '.join(map(str,pitches))+'};'
print 'uint16_t CHROM_DOWN_unique [NUNIQUE] = {'+', '.join(map(str,unique))+'};'
print 'const uint8_t  CHROM_DOWN_notes  [NNOTES]  PROGMEM = {'+', '.join(map(str,notes))+'};'
