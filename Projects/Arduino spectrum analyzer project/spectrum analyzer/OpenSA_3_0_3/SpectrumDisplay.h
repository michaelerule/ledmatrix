...
#define REFRESHINTERVAL  120        //!< For CTC interrupt at OCR0A. Driver triggers every REFRESHINTERVAL tics.
//#define REFRESHINTERVAL  80        //!< For CTC interrupt at OCR0A. Driver triggers every REFRESHINTERVAL tics.
...
#define BITDEPTH         3          //!< greyscale bit depth of internal display memory
...
void (*display_onetime_callback)() = 0; //!< register a callback function that can be polled in time with the display
...
volatile uint8_t please_flip_buffers = 0;
  ...
  if (++current_column>=NCOLS) {
    ...
    if (++current_frame_counter>=(1<<current_frame)) {
      ...
      if (++current_frame>=BITDEPTH) {
      	...
      	if (please_flip_buffers) {
      		display_buffer_type *temp = display_buffer;
			display_buffer = drawing_buffer;
			drawing_buffer = temp;
			please_flip_buffers = 0;
		}
      	//if (display_onetime_callback) display_onetime_callback();
      	//display_onetime_callback = 0;
  	  }
    }
  }
  PORTD = LATCHMASK; 
