...
volatile uint8_t sound_on   = 0;

volatile uint8_t sound_mode = 0;
        ...
        if (notecounter) notecounter--;
            ...
            case 1:
				if (noteindex>=BLUES_NNOTES) {
					...
					info = BLUES_unique[pgm_read_byte_near(&BLUES_notes[noteindex])];
					note = BLUES_pitches[info&0xff];
					notecounter = 2500+1000*noteindex;
				}
			    break;
			case 2:
				if (noteindex>=CHROM_UP_NNOTES) {
					...
					info = CHROM_UP_unique[pgm_read_byte_near(&CHROM_UP_notes[noteindex])];
			...
			case 3:
				if (noteindex>=CHROM_DOWN_NNOTES) {
					...
					info = CHROM_DOWN_unique[pgm_read_byte_near(&CHROM_DOWN_notes[noteindex])];
			...
			default:
				...
				info = unique[pgm_read_byte_near(&notes[noteindex])];
				note = pitches[info&0xff];
		        notecounter = (uint32_t)(info&0xff00)*6;
...
int main()
{    
    OSCCAL = 220; //oh dear. overclocking
    
    uint32_t i,j;
	int32_t ii = 0;
	
    TCCR1B |= 1<<WGM12;  // turn on compare to counter mode for timer 1
    TIMSK  |= 1<<OCIE1A; // turn on the output compare interrupt for timer 1A
    sei();
    OCR1AH = 1; //period
    OCR1AL = 100; //period
		...
		while(1) {
		    ...
		    while (wait)
	        {
			    ...
			    if (buttons&11) {
				    ...
				    if (can_place_block(newrow,newcolumn,neworient,block)) 
				        setBlock(r=newrow,c=newcolumn,o=neworient,block,1);
			    }
			    else if (buttons&4)
			    { 
