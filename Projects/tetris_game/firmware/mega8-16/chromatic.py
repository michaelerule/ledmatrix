...
print '#define  CHROM_NNOTES  %d'%len(notes)
print '#define  CHROM_NUNIQUE %d'%len(unique)
print '#define  CHROM_NPITCH  %d'%len(pitches)
print 'uint16_t CHROM_pitches[NPITCH]  = {'+', '.join(map(str,pitches))+'};'
print 'uint16_t CHROM_unique [NUNIQUE] = {'+', '.join(map(str,unique))+'};'
print 'const uint8_t  CHROM_notes  [NNOTES]  PROGMEM = {'+', '.join(map(str,notes))+'};'
