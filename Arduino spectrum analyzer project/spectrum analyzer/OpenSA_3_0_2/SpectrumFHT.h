/**
 * FHT for arduino - fast hartley transform.
 * @file   SpectrumFHT.h
 * openmusiclabs.com 9.1.12
 * this is a speed optimized program
 * for calculating an N point FHT on a block of data
 * Please see http://wiki.openmusiclabs.com/wiki/ArduinoFHT
 */

#ifndef _spectrum_fht_h // include guard
#define _spectrum_fht_h
...
extern prog_int16_t _cas_constants[FHT_N];
extern prog_uint8_t _reorder_table[FHT_N];
extern prog_uint8_t _log_table[FHT_N];
extern uint8_t fht_log_out[(FHT_N/2)]; // FHT log output magintude buffer
extern prog_int16_t _window_func[FHT_N];
extern int fht_input[(FHT_N)]; // FHT input data buffer

inline void fht_run(void) {
...
inline void fht_mag_octave(void) {
  // save registers that are getting clobbered
