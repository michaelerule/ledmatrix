...
const uint8_t band_edges[] = {2,3,4,5,7,8,9,11,15,20,24,32,44,56,76,96,127};
...
void setLevels(int channel)
{
  ...
  for (i=0;i<8;i++)
  {
    uint8_t k = i+2;
    uint16_t average = 0;
    average = fht_log_out[band_edges[i*2]];
    /*
    uint8_t naveraged = 0;
    for (uint8_t j=band_edges[i*2]; j<=band_edges[i*2+1]; j++)
    {
      average += fht_log_out[band_edges[j]];
      naveraged ++;
    }
    */
    //average += naveraged/2;
  ...
  for (;i<16;i++)
  {
    uint8_t k = i+2;
    uint16_t average = 0;
    average = fht_upper_log_out[band_edges[(i-8)*2]];
    /*
    uint8_t naveraged = 0;
    for (uint8_t j=band_edges[(i-8)*2]; j<=band_edges[(i-8)*2+1]; j++)
    {
      average += fht_log_out[band_edges[j]];
      naveraged ++;
    }
